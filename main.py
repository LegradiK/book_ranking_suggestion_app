...
#     else:
#         book_database.execute(
#             "INSERT INTO books (ol_key, author, title, year, genre, readinglog) VALUES (?, ?, ?, ?, ?)",
#             (ol_key, author, title, year, genre)
#         )


def get_and_insert_readinglog():
    # url = BASE_URL+ol_key
    url= "https://openlibrary.org/works/OL82563W/"
    response = requests.get(url, headers={"User-Agent": "Mozilla/5.0"})
    soup = BeautifulSoup(response.text, "html.parser")
    # finding all reading-log-stats
    stats = soup.find_all(class_="reading-log-stat")
    readinglog_total = 0
    # it's calculating numbers 3 times, needs to be fixed
    for stat in stats:
        # extract only digits, remove commas e.g. '1,580' -> 1580
        number = int(re.sub(r"[^\d]", "", stat.get_text()))
        print(number)
        readinglog_total += number
        print(readinglog_total)
    print(readinglog_total)


get_and_insert_readinglog()
